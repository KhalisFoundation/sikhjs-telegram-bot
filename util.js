...
const { buildApiUrl, SOURCES } = require('khajana');
  
const granthLabels = { sggs: 'G', dg: 'D' };

const khajana = options => fetch(buildApiUrl(options)).then(r => r.json());

const toLine = shabad => [shabad.gurbani.unicode, shabad.translation.english.ssk].join('\n    ');
...
const toAngParts = ({ page, source: { english: source, pageno } }) => [
  `${source} ${pageno}`,
  page.slice(0, page.length/2).map(({ shabad }) => toLine(shabad)).join('\n\n'),
  page.slice(page.length/2).map(({ shabad }) => toLine(shabad)).join('\n\n'),
];
